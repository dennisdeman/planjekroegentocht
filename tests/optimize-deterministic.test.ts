...
import { optimizePlanLocalIterative } from "../packages/core/src/generator";

function fixture(): { config: ConfigV2; plan: PlanV2 } {
  const config: ConfigV2 = {
    id: "cfg-opt-deterministic",
...
test("optimizer is deterministic for identical input", () => {
  ...
  const runA = optimizePlanLocalIterative(config, plan, { maxIterations: 50 });
  const runB = optimizePlanLocalIterative(config, plan, { maxIterations: 50 });

  const stationLayoutA = runA.plan.allocations
    .map((allocation) => `${allocation.id}:${allocation.stationId}`)
    .sort();
  const stationLayoutB = runB.plan.allocations
    .map((allocation) => `${allocation.id}:${allocation.stationId}`)
    .sort();

  assert.deepEqual(stationLayoutA, stationLayoutB);
  assert.deepEqual(runA.appliedMoves, runB.appliedMoves);
  assert.equal(runA.afterScore, runB.afterScore);
});
